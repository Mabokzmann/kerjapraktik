...
#Logger
logging.basicConfig(filename=r"log/bot.log", format="%(asctime)s in %(funcName)s\n  %(message)s")
...
async def show_data(update, context):
    ...
    if verify(update.message.from_user.id):
        await update.message.reply_text("Mohon maaf, User ID %s milik Anda tidak terdaftar dalam sistem kami."\
    ...
    try:
        ...
        #Error
        logging.warning(traceback.format_exc().replace("\n", "\n  "))
        await update.message.reply_text("Mohon maaf, terjadi kesalahan sistem saat sedang memproses data. "\
...
async def get_csv(update, context):
    ...
    try:
        ...
        if os.path.getsize(path) >= 48000000:
            await update.message.reply_text("Mohon maaf, berkas csv melebihi 50 MB. Silahkan menggunakan perintah /get_drive.")
        else:
            with open(path, "rb") as csv_file:
                await update.message.reply_document(csv_file, read_timeout=120, write_timeout=120, connect_timeout=120)
    #Error
    except Exception as error:
        logging.warning(traceback.format_exc().replace("\n", "\n  "))
        await update.message.reply_text("Mohon maaf, terjadi kesalahan sistem saat sedang memproses data. "\
